...
import json
import caos2 as caos
from ca1sys import CA1Sys

# --- CA-OS export ---
m = CA1Sys(fb_addr=caos.FB, fb_w=caos.W, fb_h=caos.H); caos.load_memory(m); m.SP=caos.STACK
prog, _ = caos.program()
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>CA-OS v2 — a whole Windows-like OS on a cellular automaton</title>
<style>
 ...
 *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.5 system-ui,Segoe UI,Roboto,sans-serif}
 ...
 <h1>CA-OS v2 <small>— a Windows-style desktop running entirely on the CA-1 cellular-automaton computer</small></h1>
 <p>Everything on this screen — the desktop, the window, the buttons, the mouse cursor, the calculator —
 is <b>drawn and run by CA-1 machine code</b> (~520,000 instructions on a full repaint (and only ~5,000 on an idle frame, thanks to dirty-rectangle rendering)) into a memory-mapped
 ...
 <div class="scwrap"><canvas id="screen" width="256" height="192" style="width:768px;height:576px"></canvas></div>
 ...
 mouse. All drawing, hit-testing, the calculator's arithmetic and number formatting are CA-1 instructions (with dirty-rectangle redraw + a software mouse cursor, so idle frames repaint only the cursor), and
 ...
 orchestrated, as in any CPU. On the <i>real</i> cellular automaton (~2.5 instr/s) one full-repaint OS frame would take ~2.4 days; this VM runs the identical machine code ~10⁸× faster so it's usable. It's a teaching computer
...
function makeVM(){const M=new Uint8Array(0x10000);let A=0,X=0,P=0,SP=(OS.SP||0x7FFF),PC=0,Z=1,C=0,N=0,ic=0;
...
open("dissemination/glider-lab14.html", "w").write(HTML)
print("wrote dissemination/glider-lab14.html", len(HTML), "bytes; OS prog", len(OS["prog"]), "instr, mem", len(OS["mem"]), "bytes")
