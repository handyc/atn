...
def truth(seeds):
    ok = tot = 0; tbl = {}
    for k in NOR:
        bits = [run(k[0], k[1], seed=s+5*(k[0]+2*k[1])) for s in seeds]
        tbl[k] = int(round(np.mean(bits))); ok += sum(b == NOR[k] for b in bits); tot += len(bits)
    return ok/tot, tbl

def main():
    print("CONFINED autonomous gate-to-gate wire: gate1 NOR -> channel -> gate2 reads it\n")
    tr, tbl = truth(range(8)); ho, tblh = truth(range(100, 110))
    print(f"  TRAIN    acc {100*tr:.0f}%  gate2 reads { {f'{k[0]}{k[1]}':v for k,v in tbl.items()} }")
    print(f"  HELD-OUT acc {100*ho:.0f}%  gate2 reads { {f'{k[0]}{k[1]}':v for k,v in tblh.items()} }")
    if ho >= 0.95:
