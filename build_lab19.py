#!/usr/bin/env python3
# build_lab19.py — glider-lab19.html: the dual Alice/Bob desktop, now running CA-OFFICE, steered
# over an encrypted classical line. Both run the identical CA-OFFICE suite locally; only Alice's
# tiny ENCRYPTED INPUT DELTAS (mouse x,y,button + keyboard key = 4 bytes) cross the wire, sealed
# against the pact CA keystream (fresh per delta). Deterministic CA-1 => Bob's desktop stays
# pixel-identical — including Writer text and spreadsheet edits. Self-contained.
import json
...
<title>Alice &amp; Bob — one shared office suite over an encrypted line</title>
<style>
 ...
 *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.5 system-ui,Segoe UI,Roboto,sans-serif}
 ...
 <h1>Alice &amp; Bob <small>— one shared CA-Office suite, steered over an encrypted classical line</small></h1>
 <p>Both machines run the <b>identical CA-Office</b> locally (sent once via the pact). Drive <b>Alice's</b> desktop —
 open Writer and <b>type</b>, edit the Sheet, use the Calc — and only her tiny <b>encrypted input deltas</b>
 (x, y, button, key = 4 bytes) cross the wire. Bob applies them to his copy; deterministic CA-1 keeps his desktop
 <b>pixel-identical</b>, text and all. <span id="selftest"></span></p>
 <div class="seedbar"><label>shared pact seed:</label><input type="text" id="seed" value="alice&lt;-&gt;bob pact 2026">
  <button id="rekey">re-derive &amp; reboot</button><label><input type="checkbox" id="drop"> drop the line</label>
 ...
 <div class="cols">
   ...
   <div class="card bob"><h2>🧑 Bob — mirror</h2><canvas class="screen" id="sb" width="256" height="192"></canvas></div>
 </div>
 <div class="line"><h3>🔐 the classical line — only encrypted input deltas cross it</h3>
   ...
   <div class="stat">pact CA (shared key tape) advances one generation per delta · gen <b id="gen">0</b></div>
 </div>
 <p class="note"><b>How:</b> each delta = <code>[x,y,button,key] XOR tap(pact,channel,seq)</code> with a fresh keystream
 per delta + a SHA-256 tag; Bob unseals with the same pact. The 49,152-byte screen never travels; ~10 encrypted
 bytes per action do. Tick "drop the line" → Bob freezes while Alice keeps working. Recognizable office miniatures
 (Writer/Sheet/Calc), all CA-1; mirrors <code>atn_spoeqi.py</code>.</p>
</div>
<script>
"use strict";
const OS=__OS__;
...
function reset(){if(raf)cancelAnimationFrame(raf);pact=buildPact($("seed").value);aliceVM=bootVM();bobVM=bootVM();
 mx=80;my=70;mb=0;pendKey=0;bobIn=[80,70,0];seq=0;sent=0;ndelta=0;lastMouse=null;$("sent").textContent=0;$("ndelta").textContent=0;$("wire").textContent="idle";drawGrids();raf=requestAnimationFrame(tick);}
const sa=$("sa");
...
function tick(){
 ...
 let bobKey=0;
 if((mouseChanged||pendKey!==0)&&!$("drop").checked){
   const plain=new Uint8Array([mx,my,mb,pendKey]),ks=tap(pact,CH,seq,4),ct=new Uint8Array(4);for(let i=0;i<4;i++)ct[i]=plain[i]^ks[i];
   ...
   const ks2=tap(pact,CH,seq,4),pl=new Uint8Array(4);for(let i=0;i<4;i++)pl[i]=ct[i]^ks2[i];
   const chk=new Uint8Array(8);chk.set(pl,0);new DataView(chk.buffer).setUint32(4,seq,true);
   if(hex(sha256(chk).slice(0,4))===hex(tag)){bobIn=[pl[0],pl[1],pl[2]];bobKey=pl[3];}
   sent+=ct.length+tag.length+2;ndelta++;lastMouse=[mx,my,mb];
   $("wire").innerHTML=`delta @ seq ${seq}: <span style="color:var(--pa)">${hex(ct)} ${hex(tag)}</span>${pendKey?" (incl. keystroke)":""} (${ct.length+tag.length+2} B)`;
...
open("dissemination/glider-lab19.html", "w").write(HTML)
print("wrote dissemination/glider-lab19.html", len(HTML), "bytes")
