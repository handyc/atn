#!/usr/bin/env python3
# build_lab21.py — glider-lab21.html: the SHOWCASE. The full CA-Office suite (Writer/Sheet/Calc/Paint/
# /Minesweeper/Clock) driven over the cut/restore-able encrypted line + zero-trust mother server of lab20,
# now with a live METRICS panel measuring exactly how much data crosses the classical line.
#
...
<title>CA-Office over an encrypted line — with live metrics on the data actually crossing it</title>
<style>
...
function renderMetrics(){const el=Math.max(0.001,(performance.now()-t0)/1000);
 ...
 const FULL=49152,videoBps=30*FULL,ratio=sent>0?(videoBps*el)/sent:0;
