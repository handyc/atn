#!/usr/bin/env python3
# build_lab27.py — glider-lab27.html: the CA-OS/2 SHOWCASE + a live ON-BOARD CA CLOCK (panel 6).
# Dual-pane Alice & Bob 32-bit CA-OS/2 (like lab24) plus this session's frontier: a glider circulating a
# torus as an autonomous clock driving a div2/div4 self-timed counter, and a cached-gather CA step (the
# hex_key optimization ported to JS) so the live panels run the real speed win. Built from lab24's stack.
...
# the on-board CA clock (this session): the exact glider rule catick.py selected — a 5-cell localized glider
# whose lap period is exactly 2x the board circumference. Packed 2-bit/cell to base64 for the live panel.
import catick, base64
def _pack2(lut):
    b = bytearray(len(lut) // 4)
    for i in range(0, len(lut), 4):
        b[i//4] = int(lut[i]) | (int(lut[i+1]) << 2) | (int(lut[i+2]) << 4) | (int(lut[i+3]) << 6)
    return base64.b64encode(bytes(b)).decode()
_glut, _grec, _gv, _gmx = catick.load_glider_rule()
LCLK_B64 = _pack2(_glut)
print(f"  glider-clock rule: {_grec['family']} cx={_grec['cx']:.3f} ({_gmx}-cell packet)")

HTML = r'''<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>CA-OS/2 (32-bit) over an encrypted line — Alice &amp; Bob, with a live on-board CA clock</title>
<style>
 ...
 *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.5 system-ui,Segoe UI,Roboto,sans-serif}
 ...
 <div id="tab-about" style="display:none"><div class="about">
  ...
  <p class="ev">⚡ These panels run a <b>cached-gather CA step</b> (the 6 hex neighbours are a fixed permutation, so the gather indices are precomputed once per board) — the same optimization just applied to the Python engine (2.5–4.6× faster). That headroom is what lets <b>panel 6</b> circulate a glider clock live in your browser.</p>
  ...
  <div class="pgrid">
    ...
    <div class="pcard pfull"><h4>6 · On-board CA clock <span class="tag">— the automaton keeps time (new)</span></h4>
      <div class="pd">A single <b>5-cell glider</b> circles this toroidal board by the rule alone — no controller. Each lap it crosses a fixed <b>tripwire</b> (the dim column): an autonomous <b>tick</b> whose period is set by the substrate itself (exactly 2× the board's circumference). Each tick toggles a CA flip-flop, so the two bits below count <b>÷2</b> and <b>÷4</b> — a sequential circuit whose tempo <em>is</em> the cellular automaton, not a host clock. The green flash is a tripwire crossing.</div>
      <canvas id="cclk" width="64" height="64" style="width:170px;height:170px;display:block;margin:6px auto;image-rendering:pixelated;background:#05070a;border:1px solid #2a3340;border-radius:5px"></canvas>
      <div class="preadout">laps <b id="k_laps">0</b> · lap period <b id="k_per">—</b> CA-steps · 2-bit count <b id="k_cnt">00</b> (bit1 bit0)</div>
      <div class="ptruth" id="k_train">count sequence: …</div></div>
  </div>
...
/* OPTIMIZED CA step (this session): the 6 hex neighbours of every cell are a FIXED toroidal permutation,
   so precompute the gather indices ONCE per board size and cache them — each step is then 6 array reads
   instead of per-cell modulo arithmetic. Bit-identical to the old roll-style step; the Python side got the
   same change (rulehub.hex_key, ~2.5-4.6x). This is what lets the live panels below run smoothly. */
const _nidx={};
...
const LO=unpack("__LO__",16384),LZ=unpack("__LZ__",16384),LW=unpack("__LW__",16384),LCLK=unpack("__LCLK__",16384);
const step=castep;
...
/* panel 6: on-board CA clock — a glider circles the torus; each lap crosses a tripwire = a tick that drives a ÷2/÷4 counter */
const KS=64,KCOL=(32+21)%KS,KMING=50;
let kB,kStep,kTicks,kLast,kPer,kPrevOn,kHist;
function kReset(){kB=new Uint8Array(KS*KS);pseed(kB,KS,KS>>1,KS>>1,5);kStep=0;kTicks=0;kLast=-999;kPer=0;kPrevOn=0;kHist=[];}
function kMass(){let m=0;for(let i=0;i<KS*KS;i++)if(kB[i]>0)m++;return m;}
function kStrip(){let m=0;for(let r=0;r<KS;r++)for(let d=-1;d<=1;d++){const c=((KCOL+d)%KS+KS)%KS;if(kB[r*KS+c]>0)m++;}return m;}
function kTick(){kB=castep(kB,LCLK,KS,KS);kStep++;
  const mass=kMass();if(mass===0||mass>40){kReset();return;}      // keep one clean circulating glider
  let pulse=0;const on=kStrip()>0?1:0;
  if(kStep>20&&on&&!kPrevOn&&(kStep-kLast)>=KMING){               // debounced rising edge = one tick per lap
    pulse=1;if(kLast>0)kPer=kStep-kLast;kLast=kStep;kTicks++;
    kHist.push(kTicks&3);if(kHist.length>24)kHist.shift();}
  kPrevOn=on;
  $("k_laps").textContent=kTicks;
  $("k_cnt").textContent=((kTicks>>1)&1)+""+(kTicks&1);
  $("k_per").textContent=kPer||"—";
  $("k_train").textContent="count sequence: "+(kHist.length?kHist.join(" "):"…");
  const ctx=$("cclk").getContext("2d"),im=ctx.createImageData(KS,KS);
  for(let p=0;p<KS*KS;p++){let R=18,Gc=22,B=30;if(kB[p]>0){R=255;Gc=210;B=127;}else if(p%KS===KCOL){R=34;Gc=52;B=70;}
    im.data[p*4]=R;im.data[p*4+1]=Gc;im.data[p*4+2]=B;im.data[p*4+3]=255;}
  const t=document.createElement("canvas");t.width=KS;t.height=KS;t.getContext("2d").putImageData(im,0,0);
  ctx.imageSmoothingEnabled=false;ctx.clearRect(0,0,KS,KS);ctx.drawImage(t,0,0);
  if(pulse){ctx.strokeStyle="#5ed18a";ctx.lineWidth=4;ctx.strokeRect(0,0,KS,KS);}}
gReset();lWrite(1);wReset();rWrite([1,0,1,1,0]);kReset();
let pfc=0;
function panelLoop(){if(aboutVisible){pfc++;gTick();if(pfc%2===0)lTick();iTick();wTick();rTick();kTick();}requestAnimationFrame(panelLoop);}
...
HTML = HTML.replace("__OS__", OSJSON).replace("__FONT__", FONTJSON).replace("__LO__", PLUTS["LO"]).replace("__LZ__", PLUTS["LZ"]).replace("__LW__", PLUTS["LW"]).replace("__LCLK__", LCLK_B64)
open("dissemination/glider-lab27.html", "w").write(HTML)
print("wrote dissemination/glider-lab27.html", len(HTML), "bytes")
