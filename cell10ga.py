#!/usr/bin/env python3
# cell10ga.py — evolve the cell10 GRAPH WIRING. A GA over networks of cell10
# boards where the genome includes each node's 3-port ROUTING (to another node's
# board / itself / the input signal / off), plus rule choice, size, and the port
# coupling weight port_w. This is the user's "connect them into a graph network"
# vision made searchable.
#
# Built-in control: port_w is a gene value that may be 0 (ports OFF). If the GA
# keeps port_w>0 with non-trivial routing, the cell-level wiring earns its keep;
# if it drives port_w->0, the ports don't help. Fitness = held-out both_acc on a
# VAL split (TEST stays clean), same honest protocol as caga.

import argparse, json, os, random, time
import numpy as np
from multiprocessing import Pool
import caca, cell10net
...
def fitness(gene):
    try:
        net = cell10net.HexNet10(gene, G["pool"], seed=1234)
        F = net.run(G["data"])[G["s"]:G["e"]]
        C = G["C"]
        ...
        # how many ports are actually wired on (diagnostic): non-off ports * port_w>0
        on = sum(1 for r in gene["routes"] for (k, _) in r if k != "off") if gene["port_w"] else 0
        return {"both_acc": both, "res_acc": res, "dim": net.dim,
                "lift": both - G["ctx_acc"], "ports_on": on, "port_w": gene["port_w"]}
    except Exception as ex:
        return {"both_acc": -1.0, "res_acc": -1.0, "dim": 0, "lift": -1.0, "err": repr(ex)}

SIDES = [16, 20, 24]; TICKS = [1, 2, 3]; RCELLS = [48, 64, 96]
DECAY = [0.0, 0.05, 0.1, 0.2]; PORTW = [0, 1, 1, 2]; NNODES = [2, 3, 4, 5]

def rand_port(i, N, rng):
    r = rng.random()
    if r < 0.25: return ["off", 0]
    if r < 0.45: return ["self", i]
    if r < 0.65: return ["input", 0]
...
def normalize(g, npool, rng):
    ...
    for i in range(n):
        ...
        for p in ports:
            if p[0] == "self": p[1] = i
            if p[0] == "node" and not (0 <= p[1] < n): p[1] = rng.randrange(n)
        fixed.append(ports)
    g["routes"] = fixed
...
def mutate(g, npool, rng):
    ...
    if rng.random() < mr + 0.1:                       # swap a rule
        i = rng.randrange(len(g["lut_ids"])); g["lut_ids"][i] = rng.randrange(npool)
    # REWIRE: the central operator — mutate port routes
    n = g["n_nodes"]
    for i in range(n):
        for p in range(3):
            if rng.random() < mr:
                g["routes"][i][p] = rand_port(i, n, rng)
    if rng.random() < mr:
...
def main():
    ...
    ap.add_argument("--out", default="cell10ga-news")
    ...
    print(f"corpus {a.file} {len(data)}B  pool {npool}  pop {a.pop} gens {a.gens}  "
          f"ctx-{a.ctx} baseline acc={base:.3f}")
    print(f"  (evolving cell10 port wiring; port_w may be 0=off as a built-in control)\n")
    ...
    with Pool(procs, initializer=init_worker, initargs=(pool7, data, a.warmup, a.ctx)) as P:
        for gen in range(1, a.gens + 1):
            ...
            br, bg = scored[0]
            dt = time.time() - t0
            print(f"gen {gen:2d}  both={br['both_acc']:.4f} (lift {br['lift']:+.4f}) "
                  f"res={br['res_acc']:.3f}  port_w={br['port_w']} ports_on={br['ports_on']}/"
                  f"{bg['n_nodes']*3}  n={bg['n_nodes']} side={bg['side']} ticks={bg['ticks']}  [{dt:.0f}s]")
            history.append({"gen": gen, "both": br["both_acc"], "lift": br["lift"],
                            "port_w": br["port_w"], "ports_on": br["ports_on"], "res": br["res_acc"]})
            json.dump({"gen": gen, "ctx_acc": base, "best_gene": bg, "best_result": br,
            ...
            pop = nxt
    print(f"\nDONE {time.time()-t0:.0f}s  best both={scored[0][0]['both_acc']:.4f} "
          f"vs ctx {base:.4f} (lift {scored[0][0]['lift']:+.4f})  port_w={scored[0][0]['port_w']} "
          f"-> {a.out}/state.json")
