...
## On-board CA clock: catick.py (bar #2) (2026-06-19)
The last orchestrated piece in every sequential demo so far (shiftreg, seqcircuit, the clocked datapath in
caclock) was the CLOCK — values were latched between stages by a host-driven topological sweep. Bar #2 makes
the timing itself the automaton: a single LOCALIZED glider on a toroidal board circulates by the rule alone
and crosses a fixed column "tripwire" once per horizontal lap = an autonomous periodic pulse train. No
boundary handling (the hex lattice wraps via np.roll), no host step-counter deciding when a tick happens —
the tick is the substrate coming back around.
KEY FIX (two false starts): glider_velocity only samples 20 ticks, so it green-lit rules whose "glider" is
really a translating FRONT whose wake fills the whole row band on a torus (302 cells, no clean pulse). The
selector now runs each candidate ~220 ticks on the torus and keeps only packets that stay LOCALIZED
(<70 cells, never die) — and reads a vertical column strip, not a seed-cell window, so the period is cleanly
circumference/|vx| and works for diagonal gliders too.
RESULT (newton cx=-0.105, a 5-cell glider, heading +66°, speed 1.10): the lap period is EXACTLY 2x the board
circumference — 48->96, 60->120, 72->144, 84->168 ticks — linear corr +1.000, exactly 2.00 ticks/cell. The
readout is a crisp repeating burst (██████··██████ then a long gap) every lap. That exact period-scaling with
circumference is the signature of a genuine circulating clock: a periodic tick generated by the cellular
automaton itself, no orchestration. HONEST SCOPE: this is the clock SOURCE (an autonomous CA oscillator with
